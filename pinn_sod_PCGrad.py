...
Modified by Assistant: Integrated PCGrad (Gradient Conflict Resolution) optimization.
"""
# -----------------------------------------------------------------------------------------------------------------------------------#
...
# PCGrad OPTIMIZER IMPLEMENTATION
class PCGrad:
    """
    PCGrad (Projected Conflicting Gradients) optimizer wrapper.
    It resolves gradient conflicts between different objectives by projecting
    conflicting gradients onto the normal plane of each other.
    """
    ...
    def pc_backward(self, objectives):
        """
        Calculate gradients for each objective, resolve conflicts, and accumulate.
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        for g_i in pc_grad:
            random.shuffle(grads) # Randomize order to avoid bias
            for g_j in grads:
                g_i_g_j = torch.dot(g_i, g_j)
                if g_i_g_j < 0:
                    # Conflict detected! Project g_i onto the normal plane of g_j
...
class DNN(nn.Module):
    ...
    def compute_pde_losses(self, x):
        """
        Compute the separate losses associated with the PDE residuals.
        Returns a LIST of losses [L_mass, L_momentum, L_energy].
        """
        y = self.net(x)
        ...
        # Return separate losses for PCGrad
        l_mass = (mass_residual**2).mean()
        l_mom = (momentum_residual**2).mean()
        l_energy = (energy_residual**2).mean()
        
        return [l_mass, l_mom, l_energy]

    def compute_ic_losses(self, x_ic, rho_ic, u_ic, p_ic):
        """
        Compute the separate losses associated with the initial conditions.
        Returns a LIST of losses [L_rho, L_p, L_u].
        """
        y_ic = self.net(x_ic)
        rho_ic_nn, p_ic_nn, u_ic_nn = y_ic[:, 0], y_ic[:, 1], y_ic[:, 2]

        l_rho = ((rho_ic_nn - rho_ic) ** 2).mean()
        l_p = ((p_ic_nn - p_ic) ** 2).mean()
        l_u = ((u_ic_nn - u_ic) ** 2).mean()
        
        return [l_rho, l_p, l_u]
...
def train_model(model, optimizer, scheduler, x_int_train, x_ic_train, rho_ic, u_ic, p_ic, epochs, device):
    """
    Train the PINN model using PCGrad.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        # 1. Compute Individual Losses (Don't sum them yet!)
        pde_losses = model.compute_pde_losses(x_int_train) # [mass, mom, energy]
        ic_losses = model.compute_ic_losses(x_ic_train, rho_ic, u_ic, p_ic) # [rho, p, u]
        
        # Combine all objectives into a list
        # We can apply weights here if desired, e.g., 10 * l for l in ic_losses
        all_objectives = pde_losses + [l * 10.0 for l in ic_losses] # Give IC slightly higher weight logic passed to PCGrad

        # 2. PCGrad Step
        # This calculates gradients for each objective, projects conflicts, and applies update
        pc_optimizer.pc_backward(all_objectives)
        pc_optimizer.step()
        ...
        # Monitoring
        if epoch % 100 == 0:
            # Re-sum for logging purposes
            loss_pde_val = sum([l.item() for l in pde_losses])
            loss_ic_val = sum([l.item() for l in ic_losses])
            total_loss_val = loss_pde_val + loss_ic_val
            print(f"Epoch {epoch:04d}: loss_pde = {loss_pde_val:.6f}, loss_ic = {loss_ic_val:.6f}, total = {total_loss_val:.6f}")

def get_exact_sod(x_np, t_val, gamma=1.4):
    """计算 Sod 激波管的解析解 (Unchanged)"""
...
def plot_comparison_with_error(t, x, rho_pred, p_pred, u_pred, t_eval=0.2):
    ...
    for i in range(3):
        ...
        axs[0, i].plot(x, pinn_data[i], color=colors[i], label='PINN (PCGrad)', alpha=0.7)
...
def main():
    ...
    # Hyperparameters
    learning_rate = 0.001        # Slightly increased LR for PCGrad usually works better
    epochs = 5000                # Reduced epochs for demo speed (PCGrad takes more time per step)
    num_x = 1000                
    num_t = 1000                
    num_ic_samples = 1000       
    num_f_samples = 11000       
    ...
    print("Starting training with PCGrad...")
